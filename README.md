...
This SnakeMake file generates filtered recombintion breakpoints for any Florida Scrub Jay (FSJ) chromosome. Every stage takes a chromosome label as a wildcard, and every
...
Using a phased, pedigree-based tree sequence for the chromosome (from
`shapeit5`/`tsinfer`-style phasing over the FSJ pedigree) and the pedigree
itself, the pipeline:
...
1. **Builds a null model.** Constructs an `msprime` `PedigreeBuilder` tree
   ...
   length, then simulates 100 tree sequences under a random seed over that fixed pedigree
   under a uniform recombination rate.
2. **Counts breakpoints in the real and null data.** For every individual, counts how
   many times each of their two haplotypes (nodes) switches tree-parent across the
   chromosome to find the location of a breakpoint.
3. **Cleans up phasing errors.** Raw breakpoint calls in the real data are
   inflated by phasing errors. Using filters flagging breakpoints: shared by full and half siblings, that are opposite-direction switches in full siblings, and fail to meet a certain distance between them and the nearest breakpoint.
   these out:
4. **Builds recombination maps** (direct mapping from the filtered real
   data breakpoints; comparison against the Romero et al. (2024) linkage
   map for the same chromosome) and checks weighted concentration of breakpoints in genic regions (genic/intergenic/exon/intron), the median distance to nearest TSS/TES vs. the null replicates, and how recombination maps differ between family sizes.
...
The pipeline needs [Snakemake](https://snakemake.readthedocs.io/) and these packages (`msprime`, `tskit`, `pandas`, `numpy`,
...
stages) is *not* run for all 2,602 individuals by default. It only runs for the IDs
...
| `real_breakpoints` | `real_id_table_chr<c>.csv`, `real_breakpoints_table_chr<c>.csv`, `qc_summary_chr<c>.txt` | pedigree-ID ↔ tree-sequence-ID lookup; raw (unfiltered) per-individual breakpoint counts; REAL vs. NULL crossovers-per-gamete |
| `sibling_filters` | `breakpoints_sibling_recurrence_chr<c>.csv`, `sibship_sharing_summary_chr<c>.csv`, `opposite_direction_switches_chr<c>.csv`, `sibling_filtered_bps_chr<c>.csv` | full-sibling recurrence + opposite-direction-switch + half-sibling recurrence filters applied; `sibling_filtered_bps` is the set used downstream |
| `distance_filter` | `distances_btwn_sibling_filtered_breakpoints_chr<c>.csv`, `distance_btwn_breakpoint_distribution_chr<c>.png`, `nearest_neighbor_cutoff_chr<c>.txt`, `all_filters_breakpoints_chr<c>.csv`, `filtered_bps_chr<c>.csv` | nearest-neighbour spacing filter (cutoff auto-detected per chromosome); `all_filters_breakpoints` is the human-readable (Mb-rounded) final table, `filtered_bps` is its full-bp-precision set used by every later stage |
...
| `family_size_maps` | `marey_by_family_size_chr<c>.png`, `breakpoints_{lt5,5to10,11to15,16to20,gt20}_chr<c>.csv` | recombination maps and breakpoint tables split by family size |
